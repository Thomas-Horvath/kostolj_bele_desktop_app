...
export async function getRecipesByCategory(typeParamName, subtypeParamName = "") {
  ...
  const orderedSubtypes = sortSubtypesByPreferredOrder(
    type.paramName,
    type.subtypes || []
  );
  ...
  return {
    type: {
      ...
      // A kategoriacim alatti alkategoria-szuro innen kapja meg a
      // gombokhoz szukseges adatokat. Korabban ez a lista kimaradt a
      // visszateresi ertekbol, emiatt a feluletnek nem volt mit kirenderelnie.
      subtypes: orderedSubtypes.map((subtype) => ({
        id: subtype.id,
        name: subtype.name,
